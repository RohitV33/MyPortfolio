    ...
    <footer className="bg-charcoal border-t border-white/5 pt-20">
      ...
      <div className="overflow-hidden border-b border-white/5 py-4 mb-20">
        <div className="marquee-track flex whitespace-nowrap">
          {[...Array(6)].map((_, i) => (
            <span key={i} className="font-syne font-black text-xs tracking-[0.4em] uppercase text-amber/20 px-10">
              {MARQUEE}
            </span>
          ))}
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 pb-20">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-20 items-start">
          ...
          <div className="grid grid-cols-2 gap-10 md:justify-items-end">
            <div>
              <p className="font-mono text-[9px] tracking-[0.4em] uppercase text-off-white/30 mb-8">Navigation</p>
              <ul className="space-y-4">
                {links.map((link) => (
                  <li key={link.href}>
                    <Link href={link.href} className="font-display font-bold text-xl text-off-white/60 hover:text-amber transition-colors">
                      {link.label}
                    </Link>
                  </li>
                ))}
            ...
            <div>
              <p className="font-mono text-[9px] tracking-[0.4em] uppercase text-off-white/30 mb-8">Socials</p>
              <ul className="space-y-4">
                {socials.map((social) => (
                  <li key={social.label}>
                    <a href={social.href} className="font-mono text-xs tracking-widest uppercase text-off-white/40 hover:text-amber transition-colors flex items-center gap-2">
        ...
        <div className="mt-16 md:mt-32 pt-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-[9px] tracking-[0.3em] uppercase text-off-white/20">Available for freelance</span>
          </div>
        </div>
      </div>
    </footer>
