...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(
        ...
        { y: 0, opacity: 1, rotateX: 0, duration: 1.2, stagger: 0.15, ease: "expo.out", delay: 0.2 }
      );
  ...
  return (
    <main className="bg-background pt-32 lg:pt-48 pb-64">
      {/* ─── Hero Section ─── */}
      <section className="px-8 md:px-24 mb-32">
        <div className="max-w-7xl mx-auto">
          <p className="projects-hero-text font-mono text-[10px] tracking-[0.4em] uppercase text-accent mb-8">
            Case Studies // Index
          </p>
          <h1 className="font-display text-[clamp(3.5rem,12vw,10rem)] font-bold leading-[0.85] tracking-tighter text-foreground perspective-1000">
            <span className="block projects-hero-text">The Complete</span>
            <span className="block projects-hero-text text-accent italic font-light">Archive.</span>
          </h1>
        </div>
      </section>

      {/* ─── Projects List ─── */}
      <section ref={listRef} className="px-8 md:px-24">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 border-t border-white/10">
            {projects.map((project, i) => (
              <Link 
                ...
                className="project-row group relative grid grid-cols-1 md:grid-cols-[80px_1fr_auto] gap-8 items-center py-16 border-b border-white/5 transition-all duration-500 hover:px-8"
              >
                ...
                <div className="relative z-10">
                  ...
                  <h2 className="font-display text-4xl md:text-6xl font-bold tracking-tight text-foreground group-hover:text-[#0D0D0D] transition-colors">
                    {project.title}
                  </h2>
                </div>
      ...
      {/* ─── Footer Accent ─── */}
      <section className="mt-64 px-8 text-center">
        <p className="font-mono text-[10px] tracking-[0.4em] uppercase text-foreground/20">
          End of Archive // More coming soon
        </p>
        <div className="mt-12 w-px h-24 bg-gradient-to-b from-foreground/10 to-transparent mx-auto" />
      </section>
    </main>
  );
}
