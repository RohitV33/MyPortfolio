...
export async function POST(request: Request) {
  try {
    ...
    const receiverEmail = process.env.CONTACT_RECEIVER_EMAIL || "verma61421st@gmail.com";
    ...
    if (!smtpUser || !smtpPass) {
      ...
      console.log(message);
      console.log("==========================================");
      console.log("💡 TIP: Define SMTP_USER and SMTP_PASS in .env.local to send real emails.");

      return NextResponse.json({
        success: true,
        simulated: true,
        message: "Submission received in local simulation mode. Configure .env.local to send actual emails.",
      });
    }
