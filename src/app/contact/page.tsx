  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(
        ...
        { y: 0, rotateX: 0, opacity: 1, duration: 1.4, stagger: 0.15, ease: "expo.out", delay: 0.2 }
      );
      ...
      gsap.fromTo(
        ...
        { y: 0, opacity: 1, filter: "blur(0px)", duration: 1.2, ease: "power3.out", delay: 0.4 }
      );
    }, containerRef);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        setErrorMessage(data.error || "Transmission encountered an anomaly. Please retry.");
        setStatus("error");
      }
    } catch (error) {
  ...
  return (
    <main ref={containerRef} className="bg-background pt-28 md:pt-32 lg:pt-48 pb-16 md:pb-32 min-h-screen relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none opacity-5">
        ...
        <div className="absolute top-36 left-8 text-[10px] font-mono tracking-widest uppercase">COORD // 25.5940.N</div>
          ...
          <div className="space-y-12 contact-fade-in">
            <div className="border-l border-accent/20 pl-6 space-y-6">
              ...
              <p className="font-body text-xl font-light text-foreground/70 leading-relaxed">
                Have a query, project scope, or general idea? Submit the transmission interface, and I will align coordinates to reply within 24 standard cycles.
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-border-subtle bg-glass-bg p-6 space-y-6 font-mono text-[11px] tracking-wider text-foreground/60">
              <div className="flex justify-between items-center pb-3 border-b border-border-subtle">
                <span className="uppercase text-foreground/30">System Node</span>
                <span className="font-bold text-accent">Active // Ready</span>
              </div>
              <div className="flex justify-between items-center pb-3 border-b border-border-subtle">
                <span className="uppercase text-foreground/30">Direct Link</span>
                <a href="mailto:verma61421st@gmail.com" className="hover:text-accent transition-colors font-bold text-foreground">
                  verma61421st@gmail.com
                </a>
              </div>
              <div className="flex justify-between items-center pb-3 border-b border-border-subtle">
                <span className="uppercase text-foreground/30">Node Location</span>
                <span className="text-foreground">India // UTC+5:30</span>
              </div>
              <div className="flex justify-between items-center pb-3 border-b border-border-subtle">
                <span className="uppercase text-foreground/30">System Time</span>
                <span className="text-foreground font-bold">{clientTime || "--:--:--"}</span>
              </div>
            ...
            <div className="space-y-4">
              <p className="font-mono text-[10px] tracking-[0.3em] uppercase text-foreground/30">
                External Relays
              </p>
              ...
              <AnimatePresence mode="wait">
                {status === "success" ? (
                  ...
                  >
                    ...
                    {isSimulated && (
                      <div className="p-5 rounded-2xl border border-amber/20 bg-amber/5 text-amber text-left max-w-md space-y-2">
                        <p className="font-mono text-[10px] uppercase tracking-wider font-bold">
                          💡 Developer Notice (Simulation Mode)
                        </p>
                        <p className="text-xs leading-relaxed opacity-80">
                          The backend API processed this submission locally. To receive real emails to <strong>verma61421st@gmail.com</strong>, copy the <code>.env.example</code> file to <code>.env.local</code> in the project directory, fill in your SMTP credentials, and restart the dev server.
                        </p>
                      </div>
                    )}
                  ...
                  >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      ...
                      <div className="space-y-2.5">
                        <label htmlFor="name" className="block font-mono text-[10px] tracking-[0.2em] uppercase text-foreground/40">
                          Full Name
                        </label>
                        <input
                          type="text"
                      ...
                      {/* Email input */}
                      <div className="space-y-2.5">
                        <label htmlFor="email" className="block font-mono text-[10px] tracking-[0.2em] uppercase text-foreground/40">
                          Email Address
                        </label>
                        <input
                          type="email"
                    ...
                    {/* Subject Input */}
                    <div className="space-y-2.5">
                      <label htmlFor="subject" className="block font-mono text-[10px] tracking-[0.2em] uppercase text-foreground/40">
                        Subject Matter
                      </label>
                      <input
                        type="text"
                    ...
                    {/* Message Input */}
                    <div className="space-y-2.5">
                      <label htmlFor="message" className="block font-mono text-[10px] tracking-[0.2em] uppercase text-foreground/40">
                        Signal Body (Message)
                      </label>
                      <textarea
                        id="message"
